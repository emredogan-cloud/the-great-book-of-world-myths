# BOOK STATS — The Great Book of World Myths

<!-- OTOMATİK ÜRETİLDİ — 04_BUILD/update_docs.py · ELLE DÜZENLEMEYİN -->

> Kapı: `phase1` · ölçüm anı için git geçmişine bakın

Buradaki her sayı **ölçülmüştür**. Hiçbiri elle yazılmadı.

## 1. Tek bakışta

| | Ölçülen | Hedef |
|---|---:|---:|
| Kilitli kültür | **22** | 22 |
| Aday kültür | 9 | ≥4 yedek |
| Kilitli hikâye | **45** | 45 |
| Aday hikâye | 59 | ≥55 |
| Yazılmış hikâye | **16** | 45 |
| Hikâye metni | 14,973 kelime | 43,000 |
| Hikâye ortalaması | 936 | 950 |
| Bant dışı hikâye | 0 | 0 |
| Kültürel not | 16 | 45 |
| Cümle ortalaması | 11.6 | 11.0–14.0 |
| Kısıtlılık taraması | 22/22 | 22/22 (muafiyetsiz) |
| Görsel | 0 | 68 |

## 2. Sayfa ve fiyat modeli

> ⚠ Model **KALİBRE** — kelime/sayfa tipografi tahmininden geliyor. Faz 1 gerçek dizgiyle ölçer (K3).

| | |
|---|---:|
| Kelime/sayfa (tahmin) | 357.5 |
| Hikâye başına faturalanan | 4 sayfa |
| Ön madde · bölüm · kültür kartı | 14 · 12 · 0 |
| Gövde | 180 |
| Arka madde | 21 |
| **Modelin sayfa sayısı** | **228** |
| Yol haritası hedefi | 230 |
| Sapma | %0.9 |
| Ciltsiz baskı maliyeti | 3.74 $ |
| Ciltsiz telif | 6.46 $ |

## 3. Sürümler

| Sürüm | Durum | Fiyat | Telif | Not |
|---|---|---:|---:|---|
| Ciltsiz (perfect bound) | lansman | 16.99 $ | 6.46 $ | Başabaş ACOS %37,8 — reklam rahat çalışır |
| Ciltli (case laminate) | lansman | 26.99 $ | 7.81 $ | Yol haritası: 'Bu kitapta ciltli LANSMANLA BİRLİKTE açılmalı |
| Kindle e-kitap | lansman | 7.99 $ | 5.14 $ | Dosya bütçesi TÜRETİLMİŞTİR, seçilmemiştir: 7,99 $ × %70 = 5 |
| Büyük punto | devre dışı | 19.99 $ | 0.00 $ | DEVRE DIŞI — karar K6/A6 |

## 4. Kültürel dağılım

| Makro bölge | Kilitli kültür |
|---|---:|
| Afrika | 4 |
| Avrupa | 4 |
| Doğu Asya | 3 |
| Okyanusya | 2 |
| Batı Asya | 2 |
| Mezoamerika | 2 |
| Kutup | 1 |
| Orta Asya ve Bozkır | 1 |
| Güneydoğu Asya | 1 |
| Güney Asya | 1 |
| Güney Amerika | 1 |

---

*Bu dosya `04_BUILD/update_docs.py` tarafından üretilir.*
