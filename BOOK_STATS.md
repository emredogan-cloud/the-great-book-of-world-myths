# BOOK STATS — The Great Book of World Myths

<!-- OTOMATİK ÜRETİLDİ — 04_BUILD/update_docs.py · ELLE DÜZENLEMEYİN -->

> Kapı: `phase0` · ölçüm anı için git geçmişine bakın

Buradaki her sayı **ölçülmüştür**. Hiçbiri elle yazılmadı.

## 1. Tek bakışta

| | Ölçülen | Hedef |
|---|---:|---:|
| Kilitli kültür | **6** | 22 |
| Aday kültür | 23 | ≥4 yedek |
| Kilitli hikâye | **0** | 45 |
| Aday hikâye | 0 | ≥55 |
| Yazılmış hikâye | **0** | 45 |
| Hikâye metni | 0 kelime | 43,000 |
| Hikâye ortalaması | 0 | 950 |
| Bant dışı hikâye | 0 | 0 |
| Kültürel not | 0 | 45 |
| Cümle ortalaması | 0.0 | 11.0–14.0 |
| Kısıtlılık taraması | 0/6 | 6/6 (muafiyetsiz) |
| Görsel | 0 | 68 |

## 2. Sayfa ve fiyat modeli

> ⚠ Model **KALİBRE DEĞİL** — kelime/sayfa tipografi tahmininden geliyor. Faz 1 gerçek dizgiyle ölçer (K3).

| | |
|---|---:|
| Kelime/sayfa (tahmin) | 361.1 |
| Hikâye başına faturalanan | 4 sayfa |
| Ön madde · bölüm · kültür kartı | 14 · 12 · 22 |
| Gövde | 180 |
| Arka madde | 21 |
| **Modelin sayfa sayısı** | **250** |
| Yol haritası hedefi | 230 |
| Sapma | %8.7 |
| Ciltsiz baskı maliyeti | 4.00 $ |
| Ciltsiz telif | 6.19 $ |

## 3. Sürümler

| Sürüm | Durum | Fiyat | Telif | Not |
|---|---|---:|---:|---|
| Ciltsiz (perfect bound) | lansman | 16.99 $ | 6.19 $ | Başabaş ACOS %37,8 — reklam rahat çalışır |
| Ciltli (case laminate) | lansman | 26.99 $ | 7.54 $ | Yol haritası: 'Bu kitapta ciltli LANSMANLA BİRLİKTE açılmalı |
| Kindle e-kitap | lansman | 7.99 $ | 5.14 $ | Dosya bütçesi TÜRETİLMİŞTİR, seçilmemiştir: 7,99 $ × %70 = 5 |
| Büyük punto | devre dışı | 19.99 $ | 0.00 $ | DEVRE DIŞI — karar K6/A6 |

## 4. Kültürel dağılım

| Makro bölge | Kilitli kültür |
|---|---:|
| Doğu Asya | 1 |
| Kutup | 1 |
| Okyanusya | 1 |
| Afrika | 1 |
| Batı Asya | 1 |
| Orta Asya ve Bozkır | 1 |

---

*Bu dosya `04_BUILD/update_docs.py` tarafından üretilir.*
