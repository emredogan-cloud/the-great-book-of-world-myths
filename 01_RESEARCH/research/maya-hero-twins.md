# The Ball Game Under the Ground — araştırma kaydı

<!-- OTOMATİK ÜRETİLDİ — 04_BUILD/research_gen.py
     Kaynak: 01_RESEARCH/story_index.json
     ELLE DÜZENLEMEYİN — bir sonraki üretimde kaybolur. -->

| Alan | Değer |
|---|---|
| **id** | `maya-hero-twins` |
| **Başlık (EN)** | The Ball Game Under the Ground |
| **Çalışma başlığı** | Yeraltındaki top oyunu |
| **Kültür** | Maya (Mezoamerika) |
| **Bölge** | The K'iche' highlands and Xibalba |
| **Bölüm** | Buz, Mısır ve Yüksek Soğuk Dağlar |
| **Sıra** | 37 |
| **Durum** | `locked` |
| **Görsel** | `story-037` |
| **Kelime hedefi** | 950 |
| **Yaşayan gelenek** | evet |

## 1. Kaynaklar

> Ölçüt: [`SOURCING_STANDARD.md`](../../SOURCING_STANDARD.md) — ≥2 bağımsız,
> ≥1 `primary`/`scholarly`, ≥1 güçlü doğrulama. Motif dizini ve **başka bir
> yeniden anlatım** bağımsız kaynak **sayılmaz**.

### Kaynak 1 · `primary` · doğrulama `fulltext`

- **Künye:** Popol Vuh, Ximénez manuscript (Newberry Library, Ayer MS 1515), the Hero Twins narrative
- **Not:** The only surviving manuscript, K'iche' with Spanish; the Hero Twins occupy the second and third parts.

### Kaynak 2 · `scholarly` · doğrulama `toc`

- **Künye:** Christenson (trans.), Popol Vuh: The Sacred Book of the Maya (University of Oklahoma Press, 2003)
- **Not:** Line-numbered translation from the K'iche' with literal parallel edition.

### Kaynak 3 · `scholarly` · doğrulama `toc`

- **Künye:** Tedlock (trans.), Popol Vuh: The Definitive Edition (Simon & Schuster, rev. ed. 1996)
- **Not:** Independent translation informed by work with contemporary K'iche' daykeepers.

**Kapı durumu:** ≥2 bağımsız ✅ · ≥1 primary/scholarly ✅ · ≥1 güçlü doğrulama ✅

## 2. Kanonik anlatım

- **Seçilen:** The Ximénez manuscript as translated by Christenson — Hunahpu and Xbalanque are summoned to Xibalba, lose, and win by losing on purpose.
- **Gerekçe:** One manuscript survives, so the choice is between translations, and two independent scholarly translations are cited rather than one. Christenson supplies the line numbering used here.
- **Bilinen varyantlar:** Christenson and Tedlock differ on the reading of several passages; Classic-period Maya vase paintings show episodes the manuscript does not narrate
- **Okura söylenecek:** Painted pots made a thousand years before the book was written show scenes from this story that the book does not include.

## 3. Kısıtlılık taraması

> **MUAFİYETSİZ** — karar K20. Bestiarium'da tarama muafiyet listeliydi
> ve liste hatalıydı (D28): en hassas etik notu taşıyan madde kapının
> **dışında** kaldı ve hiçbir yerde hata görünmedi.

- **Tarandı:** ✅
- **Sonuç:** Published sixteenth-century K'iche' text with a surviving manuscript and multiple critical translations; not restricted. Maya peoples are living peoples and K'iche' is a living language, so present tense is used for them. Excluded: contemporary aj q'ij daykeeping practice, calendar divination instruction, and community-specific sacred-site knowledge.

## 4. Yaş uyarlaması

> Ölçüt: [`AGE_POLICY.md`](../../AGE_POLICY.md)

| İçerik işareti | Politika seviyesi |
|---|---|
| death | `ALLOW` |
| violence | `IMPLY` |
| monsters | `ALLOW` |
| supernatural-horror | `IMPLY` |

- **Uyarlama notu:** The twins are killed more than once and the lords of Xibalba set lethal traps. AGE_POLICY §2.2 allows death plainly and §2.1 keeps the killings to their outcome. The houses of knives, cold and bats are rendered by what they do, not by what they look like inside (§2.14, §2.17). The final trick, in which the lords ask to be killed, is kept because it is the story's point.
- **İnceleme durumu:** `needs-review`

## 5. Kişiler

| Ad | Alternatif | Rol | Sözlükte |
|---|---|---|---|
| Hunahpu | Hunahpú | One of two brothers who are very good at a ball game | evet |
| Xbalanque | Xbalanqué | The other one | evet |
| One Death | Hun Came | A lord of Xibalba | evet |
| Seven Death | Vucub Came | Another | evet |

## 6. Telaffuz

> **Telaffuz uydurulmaz** — SOURCING_STANDARD § 8. Kaynağı yoksa yazılmaz;
> ad değişir veya hikâye değişir.

| Ad | Telaffuz | IPA | Kaynak |
|---|---|---|---|
| Hunahpu | hoo-nah-POO | /hunaxˈpu/ | Christenson, Popol Vuh: The Sacred Book of the Maya (2003), pronunciation guide in the introduction; K'iche' stress falls on the final syllable |
| Xbalanque | shbah-lahn-KEH | /ʃɓalanˈqe/ | Christenson, Popol Vuh: The Sacred Book of the Maya (2003), pronunciation guide in the introduction; K'iche' stress falls on the final syllable |
| Xibalba | shee-bahl-BAH | /ʃiɓalˈɓa/ | Christenson, Popol Vuh: The Sacred Book of the Maya (2003), pronunciation guide in the introduction; K'iche' stress falls on the final syllable |
| K'iche' | kee-CHEH | /kʼiˈtʃeʔ/ | Christenson, Popol Vuh: The Sacred Book of the Maya (2003), pronunciation guide in the introduction; K'iche' stress falls on the final syllable |

## 7. Olay örgüsü — dört hareket

- **① Kapı:** Two brothers play ball on the roof of the underworld, and the noise annoys the lords below.
- **② Baskı:** They are summoned down, and every house they are put in for the night is designed to kill them.
- **③ Dönüm:** They work out that the only way to beat the lords is to let themselves be beaten first.
- **④ Sonuç:** They come back, and the lords ask for a trick that cannot be done twice.

## 8. Olgusal iddialar ve kaynakları

*—*

## 9. Temalar ve motifler

- **Temalar:** games, cleverness, death and return, brothers
- **Motif kodu** (bilgi — **kapı değil**): A1300, E1

---

*Bu dosya `04_BUILD/research_gen.py` tarafından üretilir.*
