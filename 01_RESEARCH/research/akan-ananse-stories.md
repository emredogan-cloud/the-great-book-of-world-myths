# How the Stories Got Their Name — araştırma kaydı

<!-- OTOMATİK ÜRETİLDİ — 04_BUILD/research_gen.py
     Kaynak: 01_RESEARCH/story_index.json
     ELLE DÜZENLEMEYİN — bir sonraki üretimde kaybolur. -->

| Alan | Değer |
|---|---|
| **id** | `akan-ananse-stories` |
| **Başlık (EN)** | How the Stories Got Their Name |
| **Çalışma başlığı** | Hikâyeler adını nasıl aldı |
| **Kültür** | Akan (Afrika) |
| **Bölge** | Asante |
| **Bölüm** | Uzun Nehir ve Orman Yolu |
| **Sıra** | 32 |
| **Durum** | `written` |
| **Görsel** | `story-032` |
| **Kelime hedefi** | 950 |
| **Yaşayan gelenek** | evet |

## 1. Kaynaklar

> Ölçüt: [`SOURCING_STANDARD.md`](../../SOURCING_STANDARD.md) — ≥2 bağımsız,
> ≥1 `primary`/`scholarly`, ≥1 güçlü doğrulama. Motif dizini ve **başka bir
> yeniden anlatım** bağımsız kaynak **sayılmaz**.

### Kaynak 1 · `primary` · doğrulama `toc`

- **Künye:** Rattray, Akan-Ashanti Folk-Tales (Clarendon Press, Oxford, 1930)
- **Not:** Anansesem recorded in Twi with facing English translation, from named Asante narrators.

### Kaynak 2 · `primary` · doğrulama `toc`

- **Künye:** Rattray, Ashanti Proverbs (Clarendon Press, 1916)
- **Not:** Independent earlier collection used to confirm idiom and the standing of Ananse in Akan speech.

### Kaynak 3 · `scholarly` · doğrulama `toc`

- **Künye:** Yankah, "The Akan Trickster Cycle: Myth or Folktale?" (Indiana University African Studies Program, 1983)
- **Not:** Ghanaian scholar's analysis of the cycle's status and performance context.

**Kapı durumu:** ≥2 bağımsız ✅ · ≥1 primary/scholarly ✅ · ≥1 güçlü doğrulama ✅

## 2. Kanonik anlatım

- **Seçilen:** Rattray’s Twi text—Ananse buys the sky god’s stories with a hornet, a python and a leopard, and the stories take his name.
- **Gerekçe:** Rattray printed the Twi alongside the English from named narrators, which lets the reading be checked. Yankah, a Ghanaian scholar, supplies the performance context that keeps the tale a performed genre rather than a printed curiosity.
- **Bilinen varyantlar:** The price the sky god names varies between tellings; some add a fairy or a mother; Caribbean Anansi cycles are a separate tradition and are not merged here
- **Okura söylenecek:** Anansi stories travelled to the Caribbean with enslaved people and changed there; those are different stories, told by different communities.

## 3. Kısıtlılık taraması

> **MUAFİYETSİZ** — karar K20. Bestiarium'da tarama muafiyet listeliydi
> ve liste hatalıydı (D28): en hassas etik notu taşıyan madde kapının
> **dışında** kaldı ve hiçbir yerde hata görünmedi.

- **Tarandı:** ✅
- **Sonuç:** Anansesem is an explicitly public genre: performed in the evening, before everyone, with children present. Not restricted. Excluded from this book is the closed material Rattray recorded in his other volumes—Golden Stool ritual, adae ceremony detail and clan-specific knowledge.

## 4. Yaş uyarlaması

> Ölçüt: [`AGE_POLICY.md`](../../AGE_POLICY.md)

| İçerik işareti | Politika seviyesi |
|---|---|
| monsters | `ALLOW` |

- **Uyarlama notu:** No content requires omission. Ananse’s captures are tricks, not violence: nothing is killed and nothing is described beyond the trick itself (AGE_POLICY §2.4).
- **İnceleme durumu:** `cleared`

## 5. Kişiler

| Ad | Alternatif | Rol | Sözlükte |
|---|---|---|---|
| Kwaku Ananse | Ananse, Anansi | A spider who wants to own all the stories | evet |
| Nyame | Onyame | The sky god, who owns them now | evet |
| Aso | — | Ananse’s wife, who works out how to do it | evet |
| Akan | — | The people whose language these stories are told in | evet |
| Twi | — | The Akan language these stories are told and written in | evet |

## 6. Telaffuz

> **Telaffuz uydurulmaz** — SOURCING_STANDARD § 8. Kaynağı yoksa yazılmaz;
> ad değişir veya hikâye değişir.

| Ad | Telaffuz | IPA | Kaynak |
|---|---|---|---|
| Kwaku Ananse | KWAH-koo ah-NAHN-seh | /kwaku ananse/ | Christaller, Dictionary of the Asante and Fante Language called Tshi (2nd ed., 1933) |
| Nyame | NYAH-meh | /ɲame/ | Christaller, Dictionary of the Asante and Fante Language called Tshi (2nd ed., 1933) |
| Aso | AH-saw | /aso/ | Christaller, Dictionary of the Asante and Fante Language called Tshi (2nd ed., 1933) |
| anansesem | ah-nahn-SEH-sem | /ananseːsɛm/ | Christaller, Dictionary of the Asante and Fante Language called Tshi (2nd ed., 1933) |
| Akan | AH-kahn | /àkán/ | Christaller, Dictionary of the Asante and Fante Language called Tshi (2nd ed., 1933) |
| Twi | CHWEE | /tɕʷi/ | Christaller, Dictionary of the Asante and Fante Language called Tshi (2nd ed., 1933) |

## 7. Olay örgüsü — dört hareket

- **① Kapı:** A spider climbs to the sky and asks to buy something that is not for sale.
- **② Baskı:** The price is a hornet swarm, a python and a leopard, and the sky god is not expecting payment.
- **③ Dönüm:** Ananse brings all three, one at a time, and each one is caught by being asked a question.
- **④ Sonuç:** The stories come down, and from then on they are called by his name.

## 8. Olgusal iddialar ve kaynakları

*—*

## 9. Temalar ve motifler

- **Temalar:** cleverness, stories, bargains, small against large
- **Motif kodu** (bilgi — **kapı değil**): K1000, A1481

---

*Bu dosya `04_BUILD/research_gen.py` tarafından üretilir.*
