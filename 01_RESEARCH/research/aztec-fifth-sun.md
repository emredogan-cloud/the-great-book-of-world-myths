# The Small God Who Jumped First — araştırma kaydı

<!-- OTOMATİK ÜRETİLDİ — 04_BUILD/research_gen.py
     Kaynak: 01_RESEARCH/story_index.json
     ELLE DÜZENLEMEYİN — bir sonraki üretimde kaybolur. -->

| Alan | Değer |
|---|---|
| **id** | `aztec-fifth-sun` |
| **Başlık (EN)** | The Small God Who Jumped First |
| **Çalışma başlığı** | İlk atlayan küçük tanrı |
| **Kültür** | Aztec (Mexica) (Mezoamerika) |
| **Bölge** | Teotihuacan |
| **Bölüm** | Buz, Mısır ve Yüksek Soğuk Dağlar |
| **Sıra** | 39 |
| **Durum** | `written` |
| **Görsel** | `story-039` |
| **Kelime hedefi** | 950 |
| **Yaşayan gelenek** | hayır |

## 1. Kaynaklar

> Ölçüt: [`SOURCING_STANDARD.md`](../../SOURCING_STANDARD.md) — ≥2 bağımsız,
> ≥1 `primary`/`scholarly`, ≥1 güçlü doğrulama. Motif dizini ve **başka bir
> yeniden anlatım** bağımsız kaynak **sayılmaz**.

### Kaynak 1 · `primary` · doğrulama `fulltext`

- **Künye:** Sahagún, Historia general de las cosas de Nueva España (Florentine Codex), Book VII, chapter 2
- **Not:** Nahuatl text with Spanish, compiled from Nahua elders and written by Nahua scribes; Biblioteca Medicea Laurenziana.

### Kaynak 2 · `scholarly` · doğrulama `toc`

- **Künye:** Anderson & Dibble (trans.), Florentine Codex: General History of the Things of New Spain, Book 7 — The Sun, Moon, and Stars (School of American Research / University of Utah, 1953–1982)
- **Not:** Standard translation from the Nahuatl.

### Kaynak 3 · `primary` · doğrulama `canon`

- **Künye:** Codex Chimalpopoca, Leyenda de los Soles (1558)
- **Not:** Independent Nahuatl manuscript giving the same succession of suns.

**Kapı durumu:** ≥2 bağımsız ✅ · ≥1 primary/scholarly ✅ · ≥1 güçlü doğrulama ✅

## 2. Kanonik anlatım

- **Seçilen:** Florentine Codex Book VII—the gods gather in the dark at Teotihuacan and the small scabbed god goes into the fire before the rich one.
- **Gerekçe:** The Florentine Codex was compiled from Nahua elders and written down by Nahua scribes in Nahuatl, and the Leyenda de los Soles confirms the account independently. Both are Nahuatl-language witnesses rather than Spanish summaries.
- **Bilinen varyantlar:** The Leyenda de los Soles orders the five suns slightly differently; The Historia de los Mexicanos por sus pinturas gives another arrangement
- **Okura söylenecek:** Nahua sources do not fully agree on the order of the earlier suns, and this book follows the fullest one.

## 3. Kısıtlılık taraması

> **MUAFİYETSİZ** — karar K20. Bestiarium'da tarama muafiyet listeliydi
> ve liste hatalıydı (D28): en hassas etik notu taşıyan madde kapının
> **dışında** kaldı ve hiçbir yerde hata görünmedi.

- **Tarandı:** ✅
- **Sonuç:** Published sixteenth-century Nahuatl material with surviving manuscripts; not restricted. Nahua peoples are living peoples and Nahuatl is spoken today. Two framing rules apply: sacrifice is presented as a cost the community accepted, never as spectacle, and the phrase 'they were like that' is forbidden.

## 4. Yaş uyarlaması

> Ölçüt: [`AGE_POLICY.md`](../../AGE_POLICY.md)

| İçerik işareti | Politika seviyesi |
|---|---|
| sacrifice | `REVIEW` |
| death | `ALLOW` |
| religious | `REVIEW` |

- **Uyarlama notu:** The gods throw themselves into a fire so the sun will move, and afterwards more blood is required. AGE_POLICY §2.7 places sacrifice at IMPLY with review: the fact and its meaning are given, the fire is one sentence, and no ceremony is described. The framing rule in §2.7 is decisive here—this is told as something that cost the gods everything, which is what the source says.
- **İnceleme durumu:** `cleared`

## 5. Kişiler

| Ad | Alternatif | Rol | Sözlükte |
|---|---|---|---|
| Nanahuatzin | Nanahuatl | The small, sick god who has nothing to offer | evet |
| Tecuciztecatl | Tecciztecatl | The rich god who was supposed to go first | evet |
| Quetzalcoatl | — | Who is watching | evet |
| Mexica | — | What the people of this story called themselves | evet |

## 6. Telaffuz

> **Telaffuz uydurulmaz** — SOURCING_STANDARD § 8. Kaynağı yoksa yazılmaz;
> ad değişir veya hikâye değişir.

| Ad | Telaffuz | IPA | Kaynak |
|---|---|---|---|
| Nanahuatzin | nah-nah-WAHT-seen | /nanaˈwaːtsin/ | Karttunen, An Analytical Dictionary of Nahuatl (1983) — vowel length marked; Nahuatl stress is regularly penultimate |
| Tecuciztecatl | teh-koo-sees-TEH-kahtl | /tekusisˈtekatɬ/ | Karttunen, An Analytical Dictionary of Nahuatl (1983) — vowel length marked; Nahuatl stress is regularly penultimate |
| Quetzalcoatl | ket-sahl-KOH-ahtl | /ketsalˈkoːaːtɬ/ | Karttunen, An Analytical Dictionary of Nahuatl (1983) — vowel length marked; Nahuatl stress is regularly penultimate |
| Teotihuacan | teh-oh-tee-WAH-kahn | /teotiˈwakan/ | Karttunen, An Analytical Dictionary of Nahuatl (1983) — vowel length marked; Nahuatl stress is regularly penultimate |
| Mexica | meh-SHEE-kah | /meˈʃiʔkaʔ/ | Karttunen, An Analytical Dictionary of Nahuatl (1983) — vowel length marked; Nahuatl stress is regularly penultimate |

## 7. Olay örgüsü — dört hareket

- **① Kapı:** The world is dark, and the gods sit around a fire deciding who will become the sun.
- **② Baskı:** A rich god is chosen and offers gold and feathers; a small sick god has only reeds.
- **③ Dönüm:** The rich god stands at the fire four times and cannot do it, and the small one runs past him.
- **④ Sonuç:** A sun comes up, and it will not move until everyone else has paid too.

## 8. Olgusal iddialar ve kaynakları

*—*

## 9. Temalar ve motifler

- **Temalar:** sacrifice, courage, who gets chosen, cost
- **Motif kodu** (bilgi — **kapı değil**): A710, A1010

---

*Bu dosya `04_BUILD/research_gen.py` tarafından üretilir.*
