# The Boy Who Got His Eyes Back — araştırma kaydı

<!-- OTOMATİK ÜRETİLDİ — 04_BUILD/research_gen.py
     Kaynak: 01_RESEARCH/story_index.json
     ELLE DÜZENLEMEYİN — bir sonraki üretimde kaybolur. -->

| Alan | Değer |
|---|---|
| **id** | `inuit-blind-boy-loon` |
| **Başlık (EN)** | The Boy Who Got His Eyes Back |
| **Çalışma başlığı** | Gözlerini geri alan çocuk |
| **Kültür** | Inuit (Kutup) |
| **Bölge** | Baffin Island and the eastern Arctic |
| **Bölüm** | Buz, Mısır ve Yüksek Soğuk Dağlar |
| **Sıra** | 36 |
| **Durum** | `locked` |
| **Görsel** | `story-036` |
| **Kelime hedefi** | 950 |
| **Yaşayan gelenek** | evet |

## 1. Kaynaklar

> Ölçüt: [`SOURCING_STANDARD.md`](../../SOURCING_STANDARD.md) — ≥2 bağımsız,
> ≥1 `primary`/`scholarly`, ≥1 güçlü doğrulama. Motif dizini ve **başka bir
> yeniden anlatım** bağımsız kaynak **sayılmaz**.

### Kaynak 1 · `primary` · doğrulama `toc`

- **Künye:** Boas, The Central Eskimo, Sixth Annual Report of the Bureau of Ethnology (Smithsonian, 1888)
- **Not:** Field ethnography; the blind boy and the loon narrative.

### Kaynak 2 · `primary` · doğrulama `toc`

- **Künye:** Rasmussen, Intellectual Culture of the Iglulik Eskimos, Report of the Fifth Thule Expedition VII(1) (1929)
- **Not:** Independent field record of the same narrative from named narrators.

### Kaynak 3 · `reference` · doğrulama `sv`

- **Künye:** Leeming, The Oxford Companion to World Mythology (2005)
- **Yer:** s.v. "Inuit mythology"

**Kapı durumu:** ≥2 bağımsız ✅ · ≥1 primary/scholarly ✅ · ≥1 güçlü doğrulama ✅

## 2. Kanonik anlatım

- **Seçilen:** The Baffin and Iglulik records—a blind boy is lied to about the bear he shot, a loon carries him underwater until he can see, and he does not forgive.
- **Gerekçe:** Two independent field records again. This narrative is chosen alongside Sedna because the two together show the range of the tradition: one is cosmological, one is domestic and sharp.
- **Bilinen varyantlar:** The relative who lies is the boy’s mother in some tellings and his grandmother in others; Endings differ in how far the boy goes
- **Okura söylenecek:** Who tells the lie changes from teller to teller, and so does how the story ends.

## 3. Kısıtlılık taraması

> **MUAFİYETSİZ** — karar K20. Bestiarium'da tarama muafiyet listeliydi
> ve liste hatalıydı (D28): en hassas etik notu taşıyan madde kapının
> **dışında** kaldı ve hiçbir yerde hata görünmedi.

- **Tarandı:** ✅
- **Sonuç:** Published field ethnography, narrative layer, not restricted. Same exclusions as the companion record: no angakkuq initiation content, no spirit-journey technique, no personal tuurngait names. Present tense for Inuit communities.

## 4. Yaş uyarlaması

> Ölçüt: [`AGE_POLICY.md`](../../AGE_POLICY.md)

| İçerik işareti | Politika seviyesi |
|---|---|
| violence | `IMPLY` |
| revenge | `ALLOW` |
| abuse | `OMIT` |

- **Uyarlama notu:** A relative starves a blind child and lies to him, and at the end he takes his revenge on her. AGE_POLICY §2.9 places abuse at OMIT, but the cruelty here is the wrong the story judges: it is told plainly in two sentences, never staged, and never framed as deserved. The revenge is stated and its cost is left visible rather than celebrated (§2.12).
- **İnceleme durumu:** `needs-review`

## 5. Kişiler

| Ad | Alternatif | Rol | Sözlükte |
|---|---|---|---|
| The boy | — | Blind, and better at listening than anyone realises | evet |
| His sister | — | Who brings him meat when she can | evet |
| The loon | — | Who takes him down and holds him under | evet |

## 6. Telaffuz

> **Telaffuz uydurulmaz** — SOURCING_STANDARD § 8. Kaynağı yoksa yazılmaz;
> ad değişir veya hikâye değişir.

| Ad | Telaffuz | IPA | Kaynak |
|---|---|---|---|
| Inuk | IN-ook | /inuk/ | Schneider, Ulirnaisigutiit: An Inuktitut-English Dictionary of Northern Quebec, Labrador and Eastern Arctic Dialects (1985); Baffin form as recorded in Boas (1888) |
| qajaq | KY-ak | /qajaq/ | Schneider, Ulirnaisigutiit: An Inuktitut-English Dictionary of Northern Quebec, Labrador and Eastern Arctic Dialects (1985); Baffin form as recorded in Boas (1888) |
| tuktu | TOOK-too | /tuktu/ | Schneider, Ulirnaisigutiit: An Inuktitut-English Dictionary of Northern Quebec, Labrador and Eastern Arctic Dialects (1985); Baffin form as recorded in Boas (1888) |

## 7. Olay örgüsü — dört hareket

- **① Kapı:** A blind boy hears a bear at the window and shoots where the sound is.
- **② Baskı:** He is told he missed, and the meat he can smell is not given to him.
- **③ Dönüm:** A loon takes him out into the lake and holds him under until the water clears his eyes.
- **④ Sonuç:** He can see what has been on the table all winter, and he does not pretend otherwise.

## 8. Olgusal iddialar ve kaynakları

*—*

## 9. Temalar ve motifler

- **Temalar:** sight, lies, revenge, family
- **Motif kodu** (bilgi — **kapı değil**): F952.4, B552

---

*Bu dosya `04_BUILD/research_gen.py` tarafından üretilir.*
