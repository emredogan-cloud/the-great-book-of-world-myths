# The Door That Was Opened by Laughing — araştırma kaydı

<!-- OTOMATİK ÜRETİLDİ — 04_BUILD/research_gen.py
     Kaynak: 01_RESEARCH/story_index.json
     ELLE DÜZENLEMEYİN — bir sonraki üretimde kaybolur. -->

| Alan | Değer |
|---|---|
| **id** | `japanese-amaterasu-cave` |
| **Başlık (EN)** | The Door That Was Opened by Laughing |
| **Çalışma başlığı** | Kahkahayla açılan kapı |
| **Kültür** | Japanese (Doğu Asya) |
| **Bölge** | The Plain of High Heaven |
| **Bölüm** | Güneşin Doğduğu Yer |
| **Sıra** | 23 |
| **Durum** | `locked` |
| **Görsel** | `story-023` |
| **Kelime hedefi** | 950 |
| **Yaşayan gelenek** | evet |

## 1. Kaynaklar

> Ölçüt: [`SOURCING_STANDARD.md`](../../SOURCING_STANDARD.md) — ≥2 bağımsız,
> ≥1 `primary`/`scholarly`, ≥1 güçlü doğrulama. Motif dizini ve **başka bir
> yeniden anlatım** bağımsız kaynak **sayılmaz**.

### Kaynak 1 · `primary` · doğrulama `canon`

- **Künye:** Kojiki, Book I, the Ama-no-Iwato (Heavenly Rock Cave) episode
- **Not:** Standard book and section division; Philippi's numbering follows the received text.

### Kaynak 2 · `scholarly` · doğrulama `toc`

- **Künye:** Philippi (trans.), Kojiki (University of Tokyo Press, 1968)
- **Not:** Standard annotated English translation with full apparatus.

### Kaynak 3 · `primary` · doğrulama `canon`

- **Künye:** Nihon Shoki, Book I
- **Not:** Independent eighth-century compilation giving several alternative versions of the same episode.

**Kapı durumu:** ≥2 bağımsız ✅ · ≥1 primary/scholarly ✅ · ≥1 güçlü doğrulama ✅

## 2. Kanonik anlatım

- **Seçilen:** The Kojiki Book I account, with the Nihon Shoki’s alternative versions noted—Amaterasu shuts herself in a cave and is drawn out by a dance and by laughter.
- **Gerekçe:** The Kojiki is eight years older and gives one continuous version; the Nihon Shoki deliberately records several competing versions side by side. Naming both shows the reader that Japanese compilers preserved disagreement on purpose.
- **Bilinen varyantlar:** The Nihon Shoki records at least four alternative versions of this episode; The mirror’s role differs between the two compilations
- **Okura söylenecek:** The Nihon Shoki does something unusual: it writes down several different versions of the same story and refuses to choose.

## 3. Kısıtlılık taraması

> **MUAFİYETSİZ** — karar K20. Bestiarium'da tarama muafiyet listeliydi
> ve liste hatalıydı (D28): en hassas etik notu taşıyan madde kapının
> **dışında** kaldı ve hiçbir yerde hata görünmedi.

- **Tarandı:** ✅
- **Sonuç:** Published eighth-century texts, translated and studied. Shinto is a living religion: Amaterasu is revered at Ise today, so the present tense is used. Shrine ritual detail, norito texts, the closed traditions around the imperial regalia and kannushi practice are not used.

## 4. Yaş uyarlaması

> Ölçüt: [`AGE_POLICY.md`](../../AGE_POLICY.md)

| İçerik işareti | Politika seviyesi |
|---|---|
| religious | `REVIEW` |
| supernatural-horror | `IMPLY` |

- **Uyarlama notu:** Susanoo’s outrage that drives Amaterasu into the cave includes flaying a horse and throwing it into her weaving hall, killing a weaving woman. The act is summarised as violence and destruction; neither the animal’s death nor the woman’s is described (AGE_POLICY §2.1, §2.2). The religious flag governs tense and framing (§2.15).
- **İnceleme durumu:** `needs-review`

## 5. Kişiler

| Ad | Alternatif | Rol | Sözlükte |
|---|---|---|---|
| Amaterasu | Amaterasu Ōmikami | The sun, who has had enough | evet |
| Susanoo | Susanoo-no-Mikoto | Her brother, who caused this | evet |
| Ame-no-Uzume | Uzume | Who dances on an upturned tub | evet |

## 6. Telaffuz

> **Telaffuz uydurulmaz** — SOURCING_STANDARD § 8. Kaynağı yoksa yazılmaz;
> ad değişir veya hikâye değişir.

| Ad | Telaffuz | IPA | Kaynak |
|---|---|---|---|
| Amaterasu | ah-mah-teh-RAH-soo | /amaterasɯ/ | Hepburn romanisation as standardised in Kenkyūsha's New Japanese-English Dictionary (5th ed.); macron marks vowel length |
| Susanoo | soo-sah-NOH-oh | /sɯsanoo/ | Hepburn romanisation as standardised in Kenkyūsha's New Japanese-English Dictionary (5th ed.); macron marks vowel length |
| Ame-no-Uzume | AH-meh-no-oo-ZOO-meh | /amenoɯzɯme/ | Hepburn romanisation as standardised in Kenkyūsha's New Japanese-English Dictionary (5th ed.); macron marks vowel length |
| Takamagahara | tah-kah-mah-gah-HAH-rah | /takamaɡahara/ | Hepburn romanisation as standardised in Kenkyūsha's New Japanese-English Dictionary (5th ed.); macron marks vowel length |

## 7. Olay örgüsü — dört hareket

- **① Kapı:** The sun goes into a cave in the rock and pulls the door shut behind her.
- **② Baskı:** The world goes dark, the crops fail, and eight million gods hold a meeting outside.
- **③ Dönüm:** Uzume dances on an upturned tub until every god is roaring with laughter.
- **④ Sonuç:** The sun opens the door a crack to see what could possibly be funny.

## 8. Olgusal iddialar ve kaynakları

*—*

## 9. Temalar ve motifler

- **Temalar:** darkness and light, anger, community, laughter
- **Motif kodu** (bilgi — **kapı değil**): A721.1

---

*Bu dosya `04_BUILD/research_gen.py` tarafından üretilir.*
