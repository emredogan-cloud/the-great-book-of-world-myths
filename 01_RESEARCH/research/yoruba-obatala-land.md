# The Chain Let Down from the Sky — araştırma kaydı

<!-- OTOMATİK ÜRETİLDİ — 04_BUILD/research_gen.py
     Kaynak: 01_RESEARCH/story_index.json
     ELLE DÜZENLEMEYİN — bir sonraki üretimde kaybolur. -->

| Alan | Değer |
|---|---|
| **id** | `yoruba-obatala-land` |
| **Başlık (EN)** | The Chain Let Down from the Sky |
| **Çalışma başlığı** | Gökten sarkıtılan zincir |
| **Kültür** | Yoruba (Afrika) |
| **Bölge** | Ilé-Ifẹ̀ |
| **Bölüm** | Uzun Nehir ve Orman Yolu |
| **Sıra** | 30 |
| **Durum** | `locked` |
| **Görsel** | `story-030` |
| **Kelime hedefi** | 950 |
| **Yaşayan gelenek** | evet |

## 1. Kaynaklar

> Ölçüt: [`SOURCING_STANDARD.md`](../../SOURCING_STANDARD.md) — ≥2 bağımsız,
> ≥1 `primary`/`scholarly`, ≥1 güçlü doğrulama. Motif dizini ve **başka bir
> yeniden anlatım** bağımsız kaynak **sayılmaz**.

### Kaynak 1 · `primary` · doğrulama `toc`

- **Künye:** Bascom, Sixteen Cowries: Yoruba Divination from Africa to the New World (Indiana University Press, 1980)
- **Not:** Field-recorded Yoruba texts with facing English, collected from named diviners; the creation narratives appear among the verses.

### Kaynak 2 · `scholarly` · doğrulama `toc`

- **Künye:** Idowu, Olódùmarè: God in Yoruba Belief (Longmans, 1962)
- **Not:** Yoruba scholar's study of the tradition from within it.

### Kaynak 3 · `scholarly` · doğrulama `toc`

- **Künye:** Bascom, Ifa Divination: Communication between Gods and Men in West Africa (Indiana University Press, 1969)

**Kapı durumu:** ≥2 bağımsız ✅ · ≥1 primary/scholarly ✅ · ≥1 güçlü doğrulama ✅

## 2. Kanonik anlatım

- **Seçilen:** The Ilé-Ifẹ̀ creation narrative as recorded by Bascom from Yoruba narrators — Ọbàtálá is sent down with earth in a snail shell, and does not finish the job.
- **Gerekçe:** Bascom recorded from named practitioners in Yoruba with facing translation, and Idowu writes as a Yoruba scholar of his own tradition. Pairing an outside ethnographer with an inside scholar is deliberate: neither is left to speak alone.
- **Bilinen varyantlar:** Some narrators give the task to Odùduwà rather than Ọbàtálá; Ifẹ̀ and Ọ̀yọ́ traditions differ on the order of events
- **Okura söylenecek:** Yoruba narrators do not agree on who came down the chain first, and the disagreement is old and serious.

## 3. Kısıtlılık taraması

> **MUAFİYETSİZ** — karar K20. Bestiarium'da tarama muafiyet listeliydi
> ve liste hatalıydı (D28): en hassas etik notu taşıyan madde kapının
> **dışında** kaldı ve hiçbir yerde hata görünmedi.

- **Tarandı:** ✅
- **Sonuç:** Bascom's texts were recorded and published with the participation of named diviners; the narrative layer is public. Used as narrative only, never as divination instruction: which odù governs what, ẹbọ prescriptions and initiation content are excluded. Òrìṣà worship continues today in Nigeria, Brazil and Cuba, so the present tense is used.

## 4. Yaş uyarlaması

> Ölçüt: [`AGE_POLICY.md`](../../AGE_POLICY.md)

| İçerik işareti | Politika seviyesi |
|---|---|
| religious | `REVIEW` |
| culturally-sensitive | `REVIEW` |

- **Uyarlama notu:** In several tellings Ọbàtálá drinks palm wine, falls asleep and leaves the work unfinished, which is also given as the origin of people born with disabilities. That aetiology is not reproduced: it would frame disabled readers as a mistake. The unfinished work is kept; the explanation attached to it is dropped, and this is recorded here because a reader may meet it elsewhere (AGE_POLICY §2.15, §2.16).
- **İnceleme durumu:** `needs-review`

## 5. Kişiler

| Ad | Alternatif | Rol | Sözlükte |
|---|---|---|---|
| Ọbàtálá | Obatala, Òrìṣà-ńlá | Sent down to make dry land out of water | evet |
| Olódùmarè | Olodumare | Who gives the task | evet |
| Odùduwà | Oduduwa | Who finishes it, in some tellings | evet |

## 6. Telaffuz

> **Telaffuz uydurulmaz** — SOURCING_STANDARD § 8. Kaynağı yoksa yazılmaz;
> ad değişir veya hikâye değişir.

| Ad | Telaffuz | IPA | Kaynak |
|---|---|---|---|
| Ọbàtálá | aw-bah-TAH-lah | /ɔbaˈtala/ | Abraham, Dictionary of Modern Yoruba (1958) — tone marks and sub-dot vowels fix the reading |
| Olódùmarè | aw-LAW-doo-mah-reh | /olodũmaˈrɛ/ | Abraham, Dictionary of Modern Yoruba (1958) — tone marks and sub-dot vowels fix the reading |
| Odùduwà | aw-DOO-doo-wah | /oduˈduwa/ | Abraham, Dictionary of Modern Yoruba (1958) — tone marks and sub-dot vowels fix the reading |
| Ilé-Ifẹ̀ | ee-LEH ee-FEH | /ileːiˈfɛ/ | Abraham, Dictionary of Modern Yoruba (1958) — tone marks and sub-dot vowels fix the reading |

## 7. Olay örgüsü — dört hareket

- **① Kapı:** There is water everywhere and nowhere to stand, and a chain is lowered from above.
- **② Baskı:** Ọbàtálá climbs down with a snail shell of earth, a hen, and a palm nut.
- **③ Dönüm:** He pours the earth, the hen scratches it wide — and then he stops, and the work is left half done.
- **④ Sonuç:** Someone else has to finish it, and the Yoruba have been arguing about who ever since.

## 8. Olgusal iddialar ve kaynakları

*—*

## 9. Temalar ve motifler

- **Temalar:** creation, unfinished work, disagreement, beginnings
- **Motif kodu** (bilgi — **kapı değil**): A810, A955

---

*Bu dosya `04_BUILD/research_gen.py` tarafından üretilir.*
