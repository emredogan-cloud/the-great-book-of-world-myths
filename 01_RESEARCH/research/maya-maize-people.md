# Made of Mud, Wood, and Finally Maize — araştırma kaydı

<!-- OTOMATİK ÜRETİLDİ — 04_BUILD/research_gen.py
     Kaynak: 01_RESEARCH/story_index.json
     ELLE DÜZENLEMEYİN — bir sonraki üretimde kaybolur. -->

| Alan | Değer |
|---|---|
| **id** | `maya-maize-people` |
| **Başlık (EN)** | Made of Mud, Wood, and Finally Maize |
| **Çalışma başlığı** | Çamurdan, ağaçtan ve sonunda mısırdan |
| **Kültür** | Maya (Mezoamerika) |
| **Bölge** | The K’iche’ highlands |
| **Bölüm** | Buz, Mısır ve Yüksek Soğuk Dağlar |
| **Sıra** | 38 |
| **Durum** | `locked` |
| **Görsel** | `story-038` |
| **Kelime hedefi** | 950 |
| **Yaşayan gelenek** | evet |

## 1. Kaynaklar

> Ölçüt: [`SOURCING_STANDARD.md`](../../SOURCING_STANDARD.md) — ≥2 bağımsız,
> ≥1 `primary`/`scholarly`, ≥1 güçlü doğrulama. Motif dizini ve **başka bir
> yeniden anlatım** bağımsız kaynak **sayılmaz**.

### Kaynak 1 · `primary` · doğrulama `fulltext`

- **Künye:** Popol Vuh, Ximénez manuscript (Newberry Library, Ayer MS 1515), the creation of humankind

### Kaynak 2 · `scholarly` · doğrulama `toc`

- **Künye:** Christenson (trans.), Popol Vuh: The Sacred Book of the Maya (Oklahoma, 2003)

### Kaynak 3 · `scholarly` · doğrulama `toc`

- **Künye:** Tedlock (trans.), Popol Vuh: The Definitive Edition (rev. ed. 1996)

**Kapı durumu:** ≥2 bağımsız ✅ · ≥1 primary/scholarly ✅ · ≥1 güçlü doğrulama ✅

## 2. Kanonik anlatım

- **Seçilen:** The Ximénez manuscript—the makers try mud, then wood, then maize, and only the third attempt can speak properly and give thanks.
- **Gerekçe:** Same manuscript, same two independent translations. This narrative is paired with the Hero Twins because the Popol Vuh is one book and showing only its adventure half would misrepresent it.
- **Bilinen varyantlar:** Christenson and Tedlock differ on the description of the wooden people; The manuscript’s account of the flood that destroys them is compressed
- **Okura söylenecek:** The makers get it wrong twice before they get it right, and the book is not embarrassed about that.

## 3. Kısıtlılık taraması

> **MUAFİYETSİZ** — karar K20. Bestiarium'da tarama muafiyet listeliydi
> ve liste hatalıydı (D28): en hassas etik notu taşıyan madde kapının
> **dışında** kaldı ve hiçbir yerde hata görünmedi.

- **Tarandı:** ✅
- **Sonuç:** Published K’iche' text with surviving manuscript; not restricted. Present tense for living Maya peoples and for the K’iche' language. Same exclusions as the companion record.

## 4. Yaş uyarlaması

> Ölçüt: [`AGE_POLICY.md`](../../AGE_POLICY.md)

| İçerik işareti | Politika seviyesi |
|---|---|
| frightening-imagery | `IMPLY` |
| death | `ALLOW` |

- **Uyarlama notu:** The wooden people are destroyed and the source describes their own tools and animals turning on them. The destruction is told; the assault is not staged (AGE_POLICY §2.1, §2.17). The failure of the earlier attempts is kept, because a creation story in which the makers do not fail is a different story.
- **İnceleme durumu:** `cleared`

## 5. Kişiler

| Ad | Alternatif | Rol | Sözlükte |
|---|---|---|---|
| Tepew | Tepeu | One of the makers | evet |
| Q’ukumatz | Gucumatz, Q’uq’umatz | The other | evet |
| Xmucane | Xmucané | The grandmother who grinds the maize | evet |

## 6. Telaffuz

> **Telaffuz uydurulmaz** — SOURCING_STANDARD § 8. Kaynağı yoksa yazılmaz;
> ad değişir veya hikâye değişir.

| Ad | Telaffuz | IPA | Kaynak |
|---|---|---|---|
| Tepew | teh-PEW | /teˈpew/ | Christenson, Popol Vuh: The Sacred Book of the Maya (2003), pronunciation guide in the introduction; K'iche' stress falls on the final syllable |
| Q’ukumatz | koo-koo-MATS | /qʼuqʼuˈmats/ | Christenson, Popol Vuh: The Sacred Book of the Maya (2003), pronunciation guide in the introduction; K'iche' stress falls on the final syllable |
| Xmucane | shmoo-kah-NEH | /ʃmukaˈne/ | Christenson, Popol Vuh: The Sacred Book of the Maya (2003), pronunciation guide in the introduction; K'iche' stress falls on the final syllable |
| K’iche’ | kee-CHEH | /kʼiˈtʃeʔ/ | Christenson, Popol Vuh: The Sacred Book of the Maya (2003), pronunciation guide in the introduction; K'iche' stress falls on the final syllable |

## 7. Olay örgüsü — dört hareket

- **① Kapı:** The makers want something that can speak and say their names back to them.
- **② Baskı:** Mud will not hold its shape, and wood holds its shape but cannot remember anything.
- **③ Dönüm:** The wooden people are wiped out, and the makers try a third time with ground maize.
- **④ Sonuç:** The new people can see all the way to the edge of the world, which turns out to be too far.

## 8. Olgusal iddialar ve kaynakları

*—*

## 9. Temalar ve motifler

- **Temalar:** creation, failure, maize, gratitude
- **Motif kodu** (bilgi — **kapı değil**): A1200, A1241

---

*Bu dosya `04_BUILD/research_gen.py` tarafından üretilir.*
