# Two Messengers and One Road — araştırma kaydı

<!-- OTOMATİK ÜRETİLDİ — 04_BUILD/research_gen.py
     Kaynak: 01_RESEARCH/story_index.json
     ELLE DÜZENLEMEYİN — bir sonraki üretimde kaybolur. -->

| Alan | Değer |
|---|---|
| **id** | `zulu-chameleon-message` |
| **Başlık (EN)** | Two Messengers and One Road |
| **Çalışma başlığı** | İki haberci, tek yol |
| **Kültür** | Zulu (Afrika) |
| **Bölge** | KwaZulu-Natal |
| **Bölüm** | Uzun Nehir ve Orman Yolu |
| **Sıra** | 34 |
| **Durum** | `written` |
| **Görsel** | `story-034` |
| **Kelime hedefi** | 950 |
| **Yaşayan gelenek** | evet |

## 1. Kaynaklar

> Ölçüt: [`SOURCING_STANDARD.md`](../../SOURCING_STANDARD.md) — ≥2 bağımsız,
> ≥1 `primary`/`scholarly`, ≥1 güçlü doğrulama. Motif dizini ve **başka bir
> yeniden anlatım** bağımsız kaynak **sayılmaz**.

### Kaynak 1 · `primary` · doğrulama `fulltext`

- **Künye:** Callaway, The Religious System of the Amazulu, Part I: Unkulunkulu (Springvale, Natal, 1868–1870)
- **Not:** Zulu text with facing English translation, taken down from named Zulu narrators.

### Kaynak 2 · `primary` · doğrulama `toc`

- **Künye:** Callaway, Nursery Tales, Traditions, and Histories of the Zulus (Springvale, Natal, 1868)
- **Not:** Companion volume; independent narrators, same recording method.

### Kaynak 3 · `reference` · doğrulama `sv`

- **Künye:** Doke & Vilakazi, Zulu-English Dictionary (Witwatersrand University Press, 1948)
- **Yer:** s.v. "unwabu"
- **Not:** Used for the chameleon's name and for the reading of proper names.

**Kapı durumu:** ≥2 bağımsız ✅ · ≥1 primary/scholarly ✅ · ≥1 güçlü doğrulama ✅

## 2. Kanonik anlatım

- **Seçilen:** Callaway’s Zulu text—Unkulunkulu sends the chameleon with word that people will live, then sends the lizard with word that they will die, and the lizard arrives first.
- **Gerekçe:** Callaway printed the Zulu and the English side by side and named his narrators, which is unusually careful for 1868 and makes the text checkable. Two of his volumes with different narrators carry the account, so the pairing is genuinely independent.
- **Bilinen varyantlar:** Narrators differ on why the chameleon delayed—eating berries, or simply being slow; Closely related messenger narratives are recorded across southern and eastern Africa
- **Okura söylenecek:** Zulu narrators disagreed about why the chameleon stopped on the way, and Callaway wrote down more than one answer.

## 3. Kısıtlılık taraması

> **MUAFİYETSİZ** — karar K20. Bestiarium'da tarama muafiyet listeliydi
> ve liste hatalıydı (D28): en hassas etik notu taşıyan madde kapının
> **dışında** kaldı ve hiçbir yerde hata görünmedi.

- **Tarandı:** ✅
- **Sonuç:** Public narrative genre (izinganekwane and origin traditions), recorded with named narrators and published in Zulu. Not restricted. Excluded: isangoma initiation knowledge, muthi preparation, and family-specific amadlozi practice.

## 4. Yaş uyarlaması

> Ölçüt: [`AGE_POLICY.md`](../../AGE_POLICY.md)

| İçerik işareti | Politika seviyesi |
|---|---|
| death | `ALLOW` |

- **Uyarlama notu:** This story explains why people die, and it does not soften it. AGE_POLICY §2.2 places death at ALLOW and forbids euphemism: the word is used, the message is plain, and the ending is not repaired. The chameleon is still on the road when the story ends.
- **İnceleme durumu:** `cleared`

## 5. Kişiler

| Ad | Alternatif | Rol | Sözlükte |
|---|---|---|---|
| Unkulunkulu | uNkulunkulu | The first and greatest, who sends both messages | evet |
| Unwabu | chameleon | The chameleon, sent first and travelling slowly | evet |
| Intulo | lizard | The lizard, sent second and running | evet |
| Zulu | — | The language this was written down in, and the people who told it | evet |
| Callaway | — | The man who printed the Zulu and the English side by side in 1868 | evet |
| ubukwebezane | — | The berry bush the chameleon stopped at | evet |

## 6. Telaffuz

> **Telaffuz uydurulmaz** — SOURCING_STANDARD § 8. Kaynağı yoksa yazılmaz;
> ad değişir veya hikâye değişir.

| Ad | Telaffuz | IPA | Kaynak |
|---|---|---|---|
| Unkulunkulu | oon-koo-loon-KOO-loo | /uŋkuluŋˈkulu/ | Doke & Vilakazi, Zulu-English Dictionary (1948); Zulu stress is regularly penultimate |
| Unwabu | oon-WAH-boo | /unˈwaɓu/ | Doke & Vilakazi, Zulu-English Dictionary (1948); Zulu stress is regularly penultimate |
| Intulo | in-TOO-loh | /inˈtʼulo/ | Doke & Vilakazi, Zulu-English Dictionary (1948); Zulu stress is regularly penultimate |
| Zulu | ZOO-loo | /ˈzuːluː/ | Doke & Vilakazi, Zulu-English Dictionary (1948); Zulu stress is regularly penultimate |
| Callaway | KAL-uh-way | /ˈkæləweɪ/ | Doke & Vilakazi, Zulu-English Dictionary (1948); Zulu stress is regularly penultimate |
| ubukwebezane | oo-boo-kweh-beh-ZAH-neh | /uɓukʷeɓezaːne/ | Doke & Vilakazi, Zulu-English Dictionary (1948); Zulu stress is regularly penultimate |

## 7. Olay örgüsü — dört hareket

- **① Kapı:** The first and greatest calls a chameleon and gives it a message for the people.
- **② Baskı:** The chameleon goes slowly, the way chameleons do, and stops on the way.
- **③ Dönüm:** A second message is sent by lizard, and the lizard runs.
- **④ Sonuç:** The lizard arrives first, and the chameleon is still coming.

## 8. Olgusal iddialar ve kaynakları

*—*

## 9. Temalar ve motifler

- **Temalar:** death, messages, slowness, what cannot be undone
- **Motif kodu** (bilgi — **kapı değil**): A1335.1, B291.1

---

*Bu dosya `04_BUILD/research_gen.py` tarafından üretilir.*
