# The Llama That Would Not Eat — araştırma kaydı

<!-- OTOMATİK ÜRETİLDİ — 04_BUILD/research_gen.py
     Kaynak: 01_RESEARCH/story_index.json
     ELLE DÜZENLEMEYİN — bir sonraki üretimde kaybolur. -->

| Alan | Değer |
|---|---|
| **id** | `andean-llama-flood` |
| **Başlık (EN)** | The Llama That Would Not Eat |
| **Çalışma başlığı** | Yemek yemeyen lama |
| **Kültür** | Andean (Güney Amerika) |
| **Bölge** | Huarochirí, in the Andes above Lima |
| **Bölüm** | Buz, Mısır ve Yüksek Soğuk Dağlar |
| **Sıra** | 41 |
| **Durum** | `written` |
| **Görsel** | `story-041` |
| **Kelime hedefi** | 950 |
| **Yaşayan gelenek** | evet |

## 1. Kaynaklar

> Ölçüt: [`SOURCING_STANDARD.md`](../../SOURCING_STANDARD.md) — ≥2 bağımsız,
> ≥1 `primary`/`scholarly`, ≥1 güçlü doğrulama. Motif dizini ve **başka bir
> yeniden anlatım** bağımsız kaynak **sayılmaz**.

### Kaynak 1 · `primary` · doğrulama `fulltext`

- **Künye:** Huarochirí Manuscript (Biblioteca Nacional de España, MS 3169), chapter 3 — the flood and the llama
- **Not:** Quechua text written down from Andean narrators around 1608; the only document of its kind.

### Kaynak 2 · `scholarly` · doğrulama `toc`

- **Künye:** Salomon & Urioste (trans.), The Huarochirí Manuscript: A Testament of Ancient and Colonial Andean Religion (University of Texas Press, 1991)
- **Not:** First complete English translation with Urioste's Quechua transcription and Salomon's commentary.

### Kaynak 3 · `scholarly` · doğrulama `toc`

- **Künye:** Taylor (ed. and trans.), Ritos y tradiciones de Huarochirí (Institut Français d'Études Andines, 2nd ed. 1999)
- **Not:** Independent critical edition and Spanish translation of the Quechua.

**Kapı durumu:** ≥2 bağımsız ✅ · ≥1 primary/scholarly ✅ · ≥1 güçlü doğrulama ✅

## 2. Kanonik anlatım

- **Seçilen:** Chapter 3 of the Huarochirí Manuscript—a llama refuses to eat because it knows the sea is coming, and its owner listens.
- **Gerekçe:** There is one manuscript, so two independent scholarly editions are cited instead of two manuscripts. The manuscript is chosen over any later Andean flood account because it is the only one written in Quechua by Andean narrators rather than summarised by a Spanish chronicler.
- **Bilinen varyantlar:** Spanish chroniclers give Andean flood accounts that differ in detail; The manuscript’s chapters were not numbered by its writers; the numbering is editorial
- **Okura söylenecek:** This is the only book of Andean stories written down in Quechua by the people who told them.

## 3. Kısıtlılık taraması

> **MUAFİYETSİZ** — karar K20. Bestiarium'da tarama muafiyet listeliydi
> ve liste hatalıydı (D28): en hassas etik notu taşıyan madde kapının
> **dışında** kaldı ve hiçbir yerde hata görünmedi.

- **Tarandı:** ✅
- **Sonuç:** Published early-colonial Quechua manuscript with two critical editions; not restricted. Quechua peoples are living peoples and the language is spoken by millions. Excluded: contemporary despacho and pago ritual procedure, and community-specific apu knowledge.

## 4. Yaş uyarlaması

> Ölçüt: [`AGE_POLICY.md`](../../AGE_POLICY.md)

| İçerik işareti | Politika seviyesi |
|---|---|
| death | `ALLOW` |

- **Uyarlama notu:** The flood kills everyone not on the mountain. The source states it and moves on; so does this telling (AGE_POLICY §2.2). No drowning is described and no reassurance is added that the sources do not give.
- **İnceleme durumu:** `cleared`

## 5. Kişiler

| Ad | Alternatif | Rol | Sözlükte |
|---|---|---|---|
| The llama | — | Who knows what is coming and will not say it politely | evet |
| Its owner | — | Who is annoyed, and then listens | evet |
| Huillcacoto | — | The one mountain high enough to stand on | evet |
| Lima | — | The city on the coast below Huarochirí | evet |

## 6. Telaffuz

> **Telaffuz uydurulmaz** — SOURCING_STANDARD § 8. Kaynağı yoksa yazılmaz;
> ad değişir veya hikâye değişir.

| Ad | Telaffuz | IPA | Kaynak |
|---|---|---|---|
| Huarochirí | wah-roh-chee-REE | /waɾotʃiˈɾi/ | Academia Mayor de la Lengua Quechua, Diccionario Quechua-Español-Quechua (2005); Quechua stress is regularly penultimate |
| llama | YAH-mah | /ˈʎama/ | Academia Mayor de la Lengua Quechua, Diccionario Quechua-Español-Quechua (2005); Quechua stress is regularly penultimate |
| Quechua | KECH-wah | /ˈqʰeʃwa/ | Academia Mayor de la Lengua Quechua, Diccionario Quechua-Español-Quechua (2005); Quechua stress is regularly penultimate |
| Wiraqucha | wee-rah-KOH-chah | /wiɾaˈqotʃa/ | Academia Mayor de la Lengua Quechua, Diccionario Quechua-Español-Quechua (2005); Quechua stress is regularly penultimate |
| Huillcacoto | weel-kah-KOH-toh | /wiʎkaˈqutu/ | Academia Mayor de la Lengua Quechua, Diccionario Quechua-Español-Quechua (2005); Quechua stress is regularly penultimate |
| Lima | LEE-mah | /ˈlima/ | Academia Mayor de la Lengua Quechua, Diccionario Quechua-Español-Quechua (2005); Quechua stress is regularly penultimate |

## 7. Olay örgüsü — dört hareket

- **① Kapı:** A man ties his llama on the best grass on the hill and the animal will not touch it.
- **② Baskı:** He hits it with a corn cob and asks what is wrong, and the llama answers him.
- **③ Dönüm:** The sea is coming up in five days, and there is one mountain high enough.
- **④ Sonuç:** They climb, the water rises to the peak and stops, and afterwards the world is very quiet.

## 8. Olgusal iddialar ve kaynakları

| İddia | Kaynak |
|---|---|
| The Huarochirí Manuscript was written in Quechua around 1608. | Huarochirí Manuscript (Biblioteca Nacional de España, MS 3169), chapter 3 — the flood and the llama |
| Quechua is spoken by millions of people across the Andes today. | Salomon & Urioste (trans.), The Huarochirí Manuscript: A Testament of Ancient and Colonial Andean Religion (University of Texas Press, 1991) |
| The manuscript gives the black tail-tip of the fox as a consequence of the flood. | Taylor (ed. and trans.), Ritos y tradiciones de Huarochirí (Institut Français d'Études Andines, 2nd ed. 1999) |

## 9. Temalar ve motifler

- **Temalar:** warning, listening, floods, animals
- **Motif kodu** (bilgi — **kapı değil**): B133.1, A1010

---

*Bu dosya `04_BUILD/research_gen.py` tarafından üretilir.*
