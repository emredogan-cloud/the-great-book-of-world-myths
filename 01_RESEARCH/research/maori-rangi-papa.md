# The Children Who Pushed Their Parents Apart — araştırma kaydı

<!-- OTOMATİK ÜRETİLDİ — 04_BUILD/research_gen.py
     Kaynak: 01_RESEARCH/story_index.json
     ELLE DÜZENLEMEYİN — bir sonraki üretimde kaybolur. -->

| Alan | Değer |
|---|---|
| **id** | `maori-rangi-papa` |
| **Başlık (EN)** | The Children Who Pushed Their Parents Apart |
| **Çalışma başlığı** | Anne babasını ayıran çocuklar |
| **Kültür** | Māori (Okyanusya) |
| **Bölge** | Aotearoa |
| **Bölüm** | Adalar Arasındaki Deniz |
| **Sıra** | 43 |
| **Durum** | `locked` |
| **Görsel** | `story-043` |
| **Kelime hedefi** | 950 |
| **Yaşayan gelenek** | evet |

## 1. Kaynaklar

> Ölçüt: [`SOURCING_STANDARD.md`](../../SOURCING_STANDARD.md) — ≥2 bağımsız,
> ≥1 `primary`/`scholarly`, ≥1 güçlü doğrulama. Motif dizini ve **başka bir
> yeniden anlatım** bağımsız kaynak **sayılmaz**.

### Kaynak 1 · `primary` · doğrulama `fulltext`

- **Künye:** Grey, Nga Mahi a nga Tupuna (Deeds of the Ancestors), Māori text (London, 1854)
- **Not:** Māori-language text; the separation of Ranginui and Papatūānuku opens the collection.

### Kaynak 2 · `primary` · doğrulama `toc`

- **Künye:** Grey, Polynesian Mythology (London, 1855)
- **Not:** Grey's English translation.

### Kaynak 3 · `scholarly` · doğrulama `toc`

- **Künye:** Best, Maori Religion and Mythology, Dominion Museum Bulletin 10 (Wellington, 1924)
- **Not:** Independent early ethnographic treatment recording variant accounts.

**Kapı durumu:** ≥2 bağımsız ✅ · ≥1 primary/scholarly ✅ · ≥1 güçlü doğrulama ✅

## 2. Kanonik anlatım

- **Seçilen:** The account opening Grey's Māori text — the sky and the earth hold each other so tightly that their children live in the dark, and Tāne pushes them apart with his legs.
- **Gerekçe:** Grey's Māori-language volume again, with Best's independent record noting variants. The separation is placed after the Māui narrative so the reader meets the cosmology through a character they already know.
- **Bilinen varyantlar:** Iwi traditions differ on which child succeeds and on the number of children; Some accounts have Tāwhirimātea never accepting the separation
- **Okura söylenecek:** Which child manages to separate them changes between iwi, and so does how many children there were.

## 3. Kısıtlılık taraması

> **MUAFİYETSİZ** — karar K20. Bestiarium'da tarama muafiyet listeliydi
> ve liste hatalıydı (D28): en hassas etik notu taşıyan madde kapının
> **dışında** kaldı ve hiçbir yerde hata görünmedi.

- **Tarandı:** ✅
- **Sonuç:** Published Māori-language text from named narrators; narrative layer, not restricted. Same exclusions as the companion record: no iwi-specific whakapapa, no karakia, no wāhi tapu. Presented as one telling among many.

## 4. Yaş uyarlaması

> Ölçüt: [`AGE_POLICY.md`](../../AGE_POLICY.md)

| İçerik işareti | Politika seviyesi |
|---|---|
| grief | `ALLOW` |
| war | `ALLOW` |

- **Uyarlama notu:** The parents' grief at being separated is permanent and the source keeps it: Ranginui's tears are the rain. AGE_POLICY §2.3 forbids resolving grief the source leaves open, so the ending does not reunite them. Tāwhirimātea's war on his brothers is stated, not staged (§2.11).
- **İnceleme durumu:** `cleared`

## 5. Kişiler

| Ad | Alternatif | Rol | Sözlükte |
|---|---|---|---|
| Ranginui | Rangi | The sky father | evet |
| Papatūānuku | Papa | The earth mother | evet |
| Tāne | Tāne-mahuta | The child who stands on his head and pushes | evet |
| Tāwhirimātea | Tāwhiri | The child who never agrees to it | evet |

## 6. Telaffuz

> **Telaffuz uydurulmaz** — SOURCING_STANDARD § 8. Kaynağı yoksa yazılmaz;
> ad değişir veya hikâye değişir.

| Ad | Telaffuz | IPA | Kaynak |
|---|---|---|---|
| Ranginui | RAH-ngee-noo-ee | /ˈɾaŋinui/ | Te Aka Māori Dictionary (Moorfield); Māori orthography is phonemic and the macron marks vowel length |
| Papatūānuku | PAH-pah-too-ah-noo-koo | /ˈpapatuːaːnuku/ | Te Aka Māori Dictionary (Moorfield); Māori orthography is phonemic and the macron marks vowel length |
| Tāne | TAH-neh | /ˈtaːnɛ/ | Te Aka Māori Dictionary (Moorfield); Māori orthography is phonemic and the macron marks vowel length |
| Tāwhirimātea | TAH-fee-ree-MAH-teh-ah | /ˈtaːɸiɾiˈmaːtɛa/ | Te Aka Māori Dictionary (Moorfield); Māori orthography is phonemic and the macron marks vowel length |

## 7. Olay örgüsü — dört hareket

- **① Kapı:** Everything alive is squeezed between two bodies that will not let go of each other.
- **② Baskı:** The children argue about it in the dark, and one of them wants to kill the parents instead.
- **③ Dönüm:** Tāne puts his shoulders on his mother and his feet on his father and straightens his legs.
- **④ Sonuç:** There is light, and there is also a brother who has not forgiven anyone.

## 8. Olgusal iddialar ve kaynakları

*—*

## 9. Temalar ve motifler

- **Temalar:** separation, light, family, grief
- **Motif kodu** (bilgi — **kapı değil**): A625.2, A701

---

*Bu dosya `04_BUILD/research_gen.py` tarafından üretilir.*
