# Eight Heads and Eight Bowls of Wine — araştırma kaydı

<!-- OTOMATİK ÜRETİLDİ — 04_BUILD/research_gen.py
     Kaynak: 01_RESEARCH/story_index.json
     ELLE DÜZENLEMEYİN — bir sonraki üretimde kaybolur. -->

| Alan | Değer |
|---|---|
| **id** | `japanese-susanoo-orochi` |
| **Başlık (EN)** | Eight Heads and Eight Bowls of Wine |
| **Çalışma başlığı** | Sekiz baş ve sekiz kâse şarap |
| **Kültür** | Japanese (Doğu Asya) |
| **Bölge** | Izumo, on the river Hi |
| **Bölüm** | Güneşin Doğduğu Yer |
| **Sıra** | 24 |
| **Durum** | `written` |
| **Görsel** | `story-024` |
| **Kelime hedefi** | 950 |
| **Yaşayan gelenek** | evet |

## 1. Kaynaklar

> Ölçüt: [`SOURCING_STANDARD.md`](../../SOURCING_STANDARD.md) — ≥2 bağımsız,
> ≥1 `primary`/`scholarly`, ≥1 güçlü doğrulama. Motif dizini ve **başka bir
> yeniden anlatım** bağımsız kaynak **sayılmaz**.

### Kaynak 1 · `primary` · doğrulama `canon`

- **Künye:** Kojiki, Book I, the Yamata-no-Orochi episode

### Kaynak 2 · `scholarly` · doğrulama `toc`

- **Künye:** Philippi (trans.), Kojiki (University of Tokyo Press, 1968)

### Kaynak 3 · `primary` · doğrulama `canon`

- **Künye:** Nihon Shoki, Book I
- **Not:** Independent witness with variant details of the serpent and the sword.

**Kapı durumu:** ≥2 bağımsız ✅ · ≥1 primary/scholarly ✅ · ≥1 güçlü doğrulama ✅

## 2. Kanonik anlatım

- **Seçilen:** The Kojiki Book I account—Susanoo, banished from heaven, finds a family losing a daughter each year and sets out eight vats of strong wine.
- **Gerekçe:** The Kojiki narrates the episode continuously; the Nihon Shoki confirms it independently with variant detail. Both are named so the reader can see the agreement is between two texts, not one text quoted twice.
- **Bilinen varyantlar:** The Nihon Shoki gives variant accounts of how the sword was found; Izumo Fudoki preserves local Izumo traditions with different emphases
- **Okura söylenecek:** The sword found inside the serpent is said to be one of Japan’s three imperial treasures, and it is not on public display.

## 3. Kısıtlılık taraması

> **MUAFİYETSİZ** — karar K20. Bestiarium'da tarama muafiyet listeliydi
> ve liste hatalıydı (D28): en hassas etik notu taşıyan madde kapının
> **dışında** kaldı ve hiçbir yerde hata görünmedi.

- **Tarandı:** ✅
- **Sonuç:** Published eighth-century texts. Shinto is living: present tense is used for present practice. The closed traditions surrounding the imperial regalia are stated as closed—the story says the sword exists and is not shown, which is what the sources support—and no detail is invented.

## 4. Yaş uyarlaması

> Ölçüt: [`AGE_POLICY.md`](../../AGE_POLICY.md)

| İçerik işareti | Politika seviyesi |
|---|---|
| monsters | `ALLOW` |
| violence | `IMPLY` |
| sacrifice | `REVIEW` |

- **Uyarlama notu:** Seven daughters have already been taken, and the eighth is about to be. AGE_POLICY §2.7 places sacrifice at IMPLY with review: the fact and its cost are told through the parents, the taking is never a scene, and the serpent is drawn by silhouette and scale rather than by inventory (§2.4).
- **İnceleme durumu:** `cleared`

## 5. Kişiler

| Ad | Alternatif | Rol | Sözlükte |
|---|---|---|---|
| Susanoo | Susanoo-no-Mikoto | A god thrown out of heaven, looking for something to do | evet |
| Kushinada-hime | Kushinada | The eighth daughter | evet |
| Yamata-no-Orochi | — | A serpent with eight heads and eight tails | evet |
| Suga | — | The place where Susanoo settles afterwards | evet |

## 6. Telaffuz

> **Telaffuz uydurulmaz** — SOURCING_STANDARD § 8. Kaynağı yoksa yazılmaz;
> ad değişir veya hikâye değişir.

| Ad | Telaffuz | IPA | Kaynak |
|---|---|---|---|
| Susanoo | soo-sah-NOH-oh | /sɯsanoo/ | Hepburn romanisation as standardised in Kenkyūsha's New Japanese-English Dictionary (5th ed.); macron marks vowel length |
| Yamata-no-Orochi | yah-MAH-tah-no-oh-ROH-chee | /jamatanoorotʃi/ | Hepburn romanisation as standardised in Kenkyūsha's New Japanese-English Dictionary (5th ed.); macron marks vowel length |
| Kushinada-hime | koo-shee-NAH-dah-hee-meh | /kɯʃinadahime/ | Hepburn romanisation as standardised in Kenkyūsha's New Japanese-English Dictionary (5th ed.); macron marks vowel length |
| Izumo | ee-ZOO-mo | /izɯmo/ | Hepburn romanisation as standardised in Kenkyūsha's New Japanese-English Dictionary (5th ed.); macron marks vowel length |
| Suga | SOO-gah | /sɯ.ɡa/ | Hepburn romanisation as standardised in Kenkyūsha's New Japanese-English Dictionary (5th ed.); macron marks vowel length |

## 7. Olay örgüsü — dört hareket

- **① Kapı:** A god walks along a river and sees a chopstick float past, so somebody lives upstream.
- **② Baskı:** He finds two parents and one daughter, and hears what happened to the other seven.
- **③ Dönüm:** He builds a fence with eight gates and puts a vat of strong wine behind each one.
- **④ Sonuç:** The serpent drinks, and something is found inside its tail that is still spoken of today.

## 8. Olgusal iddialar ve kaynakları

| İddia | Kaynak |
|---|---|
| The sword found in the serpent is kept and is never displayed. | Kojiki, Book I, the Yamata-no-Orochi episode |
| Izumo Taisha is among the oldest shrines in Japan. | Nihon Shoki, Book I |
| Seven daughters were taken in seven successive years before the eighth. | Philippi (trans.), Kojiki (University of Tokyo Press, 1968) |

## 9. Temalar ve motifler

- **Temalar:** monsters, cunning, rescue, exile
- **Motif kodu** (bilgi — **kapı değil**): B11.2.3.1, K871

---

*Bu dosya `04_BUILD/research_gen.py` tarafından üretilir.*
