# The Baby Left on the Mountain — araştırma kaydı

<!-- OTOMATİK ÜRETİLDİ — 04_BUILD/research_gen.py
     Kaynak: 01_RESEARCH/story_index.json
     ELLE DÜZENLEMEYİN — bir sonraki üretimde kaybolur. -->

| Alan | Değer |
|---|---|
| **id** | `persian-zal-simorgh` |
| **Başlık (EN)** | The Baby Left on the Mountain |
| **Çalışma başlığı** | Dağa bırakılan bebek |
| **Kültür** | Persian (Batı Asya) |
| **Bölge** | Mount Alborz and the court of Zabulistan |
| **Bölüm** | İki Nehir Arasında |
| **Sıra** | 13 |
| **Durum** | `written` |
| **Görsel** | `story-013` |
| **Kelime hedefi** | 950 |
| **Yaşayan gelenek** | evet |

## 1. Kaynaklar

> Ölçüt: [`SOURCING_STANDARD.md`](../../SOURCING_STANDARD.md) — ≥2 bağımsız,
> ≥1 `primary`/`scholarly`, ≥1 güçlü doğrulama. Motif dizini ve **başka bir
> yeniden anlatım** bağımsız kaynak **sayılmaz**.

### Kaynak 1 · `primary` · doğrulama `canon`

- **Künye:** Ferdowsi, Shāhnāmeh, the reign of Manuchehr — the birth of Zal and the Simorgh episode
- **Not:** Khaleghi-Motlagh critical edition (Bibliotheca Persica, 1988–2008), vol. I.

### Kaynak 2 · `scholarly` · doğrulama `toc`

- **Künye:** Khaleghi-Motlagh (ed.), Shāhnāmeh, critical edition, Bibliotheca Persica (1988–2008)
- **Not:** Standard critical edition of the Persian text.

### Kaynak 3 · `reference` · doğrulama `sv`

- **Künye:** Yarshater (ed.), Encyclopædia Iranica
- **Yer:** s.v. "Zāl"

**Kapı durumu:** ≥2 bağımsız ✅ · ≥1 primary/scholarly ✅ · ≥1 güçlü doğrulama ✅

## 2. Kanonik anlatım

- **Seçilen:** Ferdowsi’s Shāhnāmeh in Khaleghi-Motlagh’s critical edition—Sam abandons his white-haired son, and the Simorgh raises him.
- **Gerekçe:** The Shāhnāmeh is the only continuous narrative source for this episode, and Khaleghi-Motlagh’s is the critical text that establishes what Ferdowsi actually wrote as against later interpolations.
- **Bilinen varyantlar:** Some manuscripts expand Sam’s dream sequence; Later Persian romances give the Simorgh a longer speaking part
- **Okura söylenecek:** Persian storytellers still perform the Shāhnāmeh aloud in teahouses; the practice is called naqqāli.

## 3. Kısıtlılık taraması

> **MUAFİYETSİZ** — karar K20. Bestiarium'da tarama muafiyet listeliydi
> ve liste hatalıydı (D28): en hassas etik notu taşıyan madde kapının
> **dışında** kaldı ve hiçbir yerde hata görünmedi.

- **Tarandı:** ✅
- **Sonuç:** Published literary epic, one thousand years in print and public performance. Not restricted. Screened separately: living Zoroastrian ritual practice is not described—the narrative layer and the worship layer are kept apart.

## 4. Yaş uyarlaması

> Ölçüt: [`AGE_POLICY.md`](../../AGE_POLICY.md)

| İçerik işareti | Politika seviyesi |
|---|---|
| abuse | `OMIT` |
| grief | `ALLOW` |

- **Uyarlama notu:** A father abandons a newborn on a mountain to die because of his appearance. AGE_POLICY §2.9 puts child abuse at OMIT; this narrative is kept because the abandonment is the wrong the story exists to condemn, it is told in two sentences without cruelty being staged, and Sam’s shame is the turn. The bird, not the father, is the parent here.
- **İnceleme durumu:** `needs-review`

## 5. Kişiler

| Ad | Alternatif | Rol | Sözlükte |
|---|---|---|---|
| Zal | Zāl, Dastān | A boy born with white hair and left on a mountain | evet |
| Simorgh | Sīmorgh | The great bird who raises him | evet |
| Sam | Sām | His father, who comes back | evet |
| Rostam | — | Zal’s son, and the hero the rest of the epic follows | evet |

## 6. Telaffuz

> **Telaffuz uydurulmaz** — SOURCING_STANDARD § 8. Kaynağı yoksa yazılmaz;
> ad değişir veya hikâye değişir.

| Ad | Telaffuz | IPA | Kaynak |
|---|---|---|---|
| Zal | ZAWL | /zɒːl/ | Steingass, A Comprehensive Persian-English Dictionary (1892), headword transliteration; modern values per Dehkhodā, Loghatnāmeh |
| Simorgh | see-MORG | /siːˈmorɣ/ | Steingass, A Comprehensive Persian-English Dictionary (1892), headword transliteration; modern values per Dehkhodā, Loghatnāmeh |
| Sam | SAWM | /sɒːm/ | Steingass, A Comprehensive Persian-English Dictionary (1892), headword transliteration; modern values per Dehkhodā, Loghatnāmeh |
| Alborz | al-BORZ | /ælˈborz/ | Steingass, A Comprehensive Persian-English Dictionary (1892), headword transliteration; modern values per Dehkhodā, Loghatnāmeh |
| Rostam | ross-TAM | /rosˈtæm/ | Steingass, A Comprehensive Persian-English Dictionary (1892), headword transliteration; modern values per Dehkhodā, Loghatnāmeh |

## 7. Olay örgüsü — dört hareket

- **① Kapı:** A man carries a newborn up a mountain because the baby's hair is white.
- **② Baskı:** A bird with a nest on the summit finds him and does not eat him.
- **③ Dönüm:** Years later the father climbs back up, and the boy has to choose.
- **④ Sonuç:** Zal comes down with a feather, and is told to burn it if he is ever desperate.

## 8. Olgusal iddialar ve kaynakları

*—*

## 9. Temalar ve motifler

- **Temalar:** abandonment, belonging, fathers, return
- **Motif kodu** (bilgi — **kapı değil**): S312, B535

---

*Bu dosya `04_BUILD/research_gen.py` tarafından üretilir.*
