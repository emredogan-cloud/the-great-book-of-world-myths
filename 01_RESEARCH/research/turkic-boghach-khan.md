# The Boy Who Fought the Bull — araştırma kaydı

<!-- OTOMATİK ÜRETİLDİ — 04_BUILD/research_gen.py
     Kaynak: 01_RESEARCH/story_index.json
     ELLE DÜZENLEMEYİN — bir sonraki üretimde kaybolur. -->

| Alan | Değer |
|---|---|
| **id** | `turkic-boghach-khan` |
| **Başlık (EN)** | The Boy Who Fought the Bull |
| **Çalışma başlığı** | Boğayla güreşen çocuk |
| **Kültür** | Turkic (Orta Asya ve Bozkır) |
| **Bölge** | The Oghuz lands |
| **Bölüm** | İki Nehir Arasında |
| **Sıra** | 15 |
| **Durum** | `written` |
| **Görsel** | `story-015` |
| **Kelime hedefi** | 950 |
| **Yaşayan gelenek** | evet |

## 1. Kaynaklar

> Ölçüt: [`SOURCING_STANDARD.md`](../../SOURCING_STANDARD.md) — ≥2 bağımsız,
> ≥1 `primary`/`scholarly`, ≥1 güçlü doğrulama. Motif dizini ve **başka bir
> yeniden anlatım** bağımsız kaynak **sayılmaz**.

### Kaynak 1 · `primary` · doğrulama `fulltext`

- **Künye:** Kitāb-i Dedem Korkut, Dresden manuscript (SLUB Dresden, Mscr.Dresd.Ea.86), tale 1: Boghach Khan son of Dirse Khan
- **Not:** The only complete manuscript witness; twelve tales, digitised by SLUB Dresden.

### Kaynak 2 · `scholarly` · doğrulama `toc`

- **Künye:** Lewis (trans.), The Book of Dede Korkut (Penguin Classics, 1974)
- **Not:** First complete English translation, with introduction and notes on the manuscript.

### Kaynak 3 · `reference` · doğrulama `toc`

- **Künye:** Ergin (ed.), Dede Korkut Kitabı, Türk Dil Kurumu (1958–1963)
- **Not:** Standard Turkish critical edition.

**Kapı durumu:** ≥2 bağımsız ✅ · ≥1 primary/scholarly ✅ · ≥1 güçlü doğrulama ✅

## 2. Kanonik anlatım

- **Seçilen:** Tale 1 of the Dresden manuscript—a boy with no name kills a bull in the assembly ground and Dede Korkut names him Boghach.
- **Gerekçe:** The Dresden manuscript is the only witness carrying all twelve tales; the Vatican manuscript holds six and does not improve this one. Ergin’s and Lewis’s editions both work from Dresden, so the choice is the manuscript itself.
- **Bilinen varyantlar:** The Vatican manuscript (Cod. Vat. Turc. 102) contains six of the twelve tales; A third manuscript found in 2018 (Günbet) contains related material
- **Okura söylenecek:** A new manuscript of these tales turned up in 2018, which shows how recently the record can still change.

## 3. Kısıtlılık taraması

> **MUAFİYETSİZ** — karar K20. Bestiarium'da tarama muafiyet listeliydi
> ve liste hatalıydı (D28): en hassas etik notu taşıyan madde kapının
> **dışında** kaldı ve hiçbir yerde hata görünmedi.

- **Tarandı:** ✅
- **Sonuç:** Published manuscript epic, listed on the UNESCO Memory of the World register and digitised by the Saxon State Library. Not restricted. Screened separately: living Siberian Turkic shamanic practice is a different restriction class and is not used here.

## 4. Yaş uyarlaması

> Ölçüt: [`AGE_POLICY.md`](../../AGE_POLICY.md)

| İçerik işareti | Politika seviyesi |
|---|---|
| violence | `IMPLY` |
| abuse | `OMIT` |

- **Uyarlama notu:** In the source Dirse Khan is turned against his son and wounds him. AGE_POLICY §2.9 flags a parent harming a child; this is kept because it is the wrong the tale resolves, the wounding is one sentence, and the mother’s search is the story’s turn. The bull fight is three sentences with a single sensory detail (§2.1).
- **İnceleme durumu:** `needs-review`

## 5. Kişiler

| Ad | Alternatif | Rol | Sözlükte |
|---|---|---|---|
| Boghach | Boğaç | A boy who is fifteen and has not earned a name yet | evet |
| Dirse Khan | Dirse Han | His father | evet |
| Dede Korkut | Dedem Korkut | The old storyteller who gives names | evet |

## 6. Telaffuz

> **Telaffuz uydurulmaz** — SOURCING_STANDARD § 8. Kaynağı yoksa yazılmaz;
> ad değişir veya hikâye değişir.

| Ad | Telaffuz | IPA | Kaynak |
|---|---|---|---|
| Boghach | boh-AHCH | /boˈatʃ/ | Türk Dil Kurumu, Güncel Türkçe Sözlük — Turkish orthography is phonemic; Oghuz forms per Lewis, The Book of Dede Korkut (1974), transliteration note |
| Dede Korkut | deh-DEH kor-KOOT | /deˈde koɾˈkut/ | Türk Dil Kurumu, Güncel Türkçe Sözlük — Turkish orthography is phonemic; Oghuz forms per Lewis, The Book of Dede Korkut (1974), transliteration note |
| Dirse Khan | DEER-seh KAHN | /ˈdiɾse/ | Türk Dil Kurumu, Güncel Türkçe Sözlük — Turkish orthography is phonemic; Oghuz forms per Lewis, The Book of Dede Korkut (1974), transliteration note |
| Oghuz | oh-OOZ | /oˈuz/ | Türk Dil Kurumu, Güncel Türkçe Sözlük — Turkish orthography is phonemic; Oghuz forms per Lewis, The Book of Dede Korkut (1974), transliteration note |

## 7. Olay örgüsü — dört hareket

- **① Kapı:** Three tents stand at a feast — white, red and black — and one man is sent to the black one.
- **② Baskı:** His shame is that he has no son; then he has one, and the boy has no name.
- **③ Dönüm:** A bull is loosed in the assembly ground and the boy takes his fist away from its forehead.
- **④ Sonuç:** The old storyteller arrives, names him Boghach, and the naming is the reward.

## 8. Olgusal iddialar ve kaynakları

*—*

## 9. Temalar ve motifler

- **Temalar:** names, coming of age, fathers and sons, reputation
- **Motif kodu** (bilgi — **kapı değil**): Z183, B871

---

*Bu dosya `04_BUILD/research_gen.py` tarafından üretilir.*
