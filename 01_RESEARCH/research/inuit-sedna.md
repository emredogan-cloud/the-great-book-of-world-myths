# The Woman at the Bottom of the Sea — araştırma kaydı

<!-- OTOMATİK ÜRETİLDİ — 04_BUILD/research_gen.py
     Kaynak: 01_RESEARCH/story_index.json
     ELLE DÜZENLEMEYİN — bir sonraki üretimde kaybolur. -->

| Alan | Değer |
|---|---|
| **id** | `inuit-sedna` |
| **Başlık (EN)** | The Woman at the Bottom of the Sea |
| **Çalışma başlığı** | Denizin dibindeki kadın |
| **Kültür** | Inuit (Kutup) |
| **Bölge** | Baffin Island and the eastern Arctic |
| **Bölüm** | Buz, Mısır ve Yüksek Soğuk Dağlar |
| **Sıra** | 35 |
| **Durum** | `written` |
| **Görsel** | `story-035` |
| **Kelime hedefi** | 950 |
| **Yaşayan gelenek** | evet |

## 1. Kaynaklar

> Ölçüt: [`SOURCING_STANDARD.md`](../../SOURCING_STANDARD.md) — ≥2 bağımsız,
> ≥1 `primary`/`scholarly`, ≥1 güçlü doğrulama. Motif dizini ve **başka bir
> yeniden anlatım** bağımsız kaynak **sayılmaz**.

### Kaynak 1 · `primary` · doğrulama `toc`

- **Künye:** Boas, The Central Eskimo, Sixth Annual Report of the Bureau of Ethnology (Smithsonian, 1888)
- **Not:** Field ethnography from Baffin Island; the Sedna narrative and its variants.

### Kaynak 2 · `primary` · doğrulama `toc`

- **Künye:** Rasmussen, Intellectual Culture of the Iglulik Eskimos, Report of the Fifth Thule Expedition VII(1) (Copenhagen, 1929)
- **Not:** Independent field record from a different region and forty years later, with named narrators.

### Kaynak 3 · `reference` · doğrulama `sv`

- **Künye:** Leeming, The Oxford Companion to World Mythology (2005)
- **Yer:** s.v. "Sedna"

**Kapı durumu:** ≥2 bağımsız ✅ · ≥1 primary/scholarly ✅ · ≥1 güçlü doğrulama ✅

## 2. Kanonik anlatım

- **Seçilen:** The Baffin Island account recorded by Boas, cross-read with Rasmussen’s Iglulik record—a woman goes into the sea and the animals people hunt come from her.
- **Gerekçe:** Two independent field records, separated by forty years and by region, with named narrators in Rasmussen’s case. Neither is a retelling of the other, which is exactly what the two-source rule is for.
- **Bilinen varyantlar:** Her name differs by region: Sedna, Nuliajuk, Takánakapsâluk, Arnakuagsak; The reason she goes into the sea differs sharply between tellings
- **Okura söylenecek:** This woman has several names depending on where the story is told, and the reasons given for what happened differ too.

## 3. Kısıtlılık taraması

> **MUAFİYETSİZ** — karar K20. Bestiarium'da tarama muafiyet listeliydi
> ve liste hatalıydı (D28): en hassas etik notu taşıyan madde kapının
> **dışında** kaldı ve hiçbir yerde hata görünmedi.

- **Tarandı:** ✅
- **Sonuç:** Published field ethnography from named narrators; the narrative layer is not restricted. Excluded: angakkuq initiation knowledge, the technique of the spirit-journey to her, personal and family tuurngait names, and amulet practice. Inuit communities are living communities and the past tense is not used for them.

## 4. Yaş uyarlaması

> Ölçüt: [`AGE_POLICY.md`](../../AGE_POLICY.md)

| İçerik işareti | Politika seviyesi |
|---|---|
| violence | `IMPLY` |
| death | `ALLOW` |
| grief | `ALLOW` |
| frightening-imagery | `IMPLY` |

- **Uyarlama notu:** In the sources her fingers are cut off as she clings to the boat, and the joints become the sea creatures. This is the centre of the story and cannot be removed without destroying it. AGE_POLICY §2.1 governs: the act is told in one sentence, the transformation is the scene, and there is no description of injury. The father is not excused and the story is not given a reconciliation the sources do not have.
- **İnceleme durumu:** `cleared`

## 5. Kişiler

| Ad | Alternatif | Rol | Sözlükte |
|---|---|---|---|
| Sedna | Nuliajuk, Takánakapsâluk | Who lives at the bottom of the sea and holds the animals | evet |
| Her father | — | Who is in the boat with her | hayır |
| The angakkuq | — | The one who has to go down and comb her hair | evet |
| Inuit | Inuk | The people whose story this is, across the whole Arctic | evet |

## 6. Telaffuz

> **Telaffuz uydurulmaz** — SOURCING_STANDARD § 8. Kaynağı yoksa yazılmaz;
> ad değişir veya hikâye değişir.

| Ad | Telaffuz | IPA | Kaynak |
|---|---|---|---|
| Sedna | SED-nuh | /ˈsɛdnə/ | Schneider, Ulirnaisigutiit: An Inuktitut-English Dictionary of Northern Quebec, Labrador and Eastern Arctic Dialects (1985); Baffin form as recorded in Boas (1888) |
| Nuliajuk | noo-lee-AH-yuk | /nuliajuk/ | Schneider, Ulirnaisigutiit: An Inuktitut-English Dictionary of Northern Quebec, Labrador and Eastern Arctic Dialects (1985); Baffin form as recorded in Boas (1888) |
| angakkuq | ah-NGAH-kook | /aŋakːuq/ | Schneider, Ulirnaisigutiit: An Inuktitut-English Dictionary of Northern Quebec, Labrador and Eastern Arctic Dialects (1985); Baffin form as recorded in Boas (1888) |
| Inuit | IN-oo-eet | /ˈinuit/ | Schneider, Ulirnaisigutiit: An Inuktitut-English Dictionary of Northern Quebec, Labrador and Eastern Arctic Dialects (1985); Baffin form as recorded in Boas (1888) |

## 7. Olay örgüsü — dört hareket

- **① Kapı:** A storm comes up while a father and daughter are far out in a small boat.
- **② Baskı:** The boat will not carry them both, and the water is taking one of them.
- **③ Dönüm:** She goes under, and what happens to her hands becomes every animal in the sea.
- **④ Sonuç:** She keeps them now, and when people are careless someone has to go down and ask.

## 8. Olgusal iddialar ve kaynakları

*—*

## 9. Temalar ve motifler

- **Temalar:** the sea, betrayal, consequence, respect
- **Motif kodu** (bilgi — **kapı değil**): A310, A1470

---

*Bu dosya `04_BUILD/research_gen.py` tarafından üretilir.*
